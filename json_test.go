...
var (
	// PersistTestingDir is the directory that contains all of the files and
	// folders created during testing.
	PersistTestingDir = filepath.Join(os.TempDir(), "PersistTesting")
)

// CopyFile copies a file from a source to a destination.
func CopyFile(source, dest string) (err error) {
	sf, err := os.Open(path.Clean(source))
	if err != nil {
		return
	}
	defer func() {
...
func TempDir(dirs ...string) string {
	path := filepath.Join(PersistTestingDir, filepath.Join(dirs...))
...
func TestSaveLoadJSON(t *testing.T) {
	...
	dir := filepath.Join(TempDir(persistDir), t.Name())
...
func TestSaveJSONCorruptedMainFile(t *testing.T) {
	...
	err = CopyFile(corruptMainFileSource, corruptMainFileDest)
	if err != nil {
		t.Fatal(err)
	}
	err = CopyFile(corruptMainTempFileSource, corruptMainTempFileDest)
	if err != nil {
		t.Fatal(err)
	}
	...
	file, err := os.Open(path.Clean(corruptMainTempFileDest))
	if err != nil {
		t.Fatal(err)
	}
	...
	if !bytes.Equal(tempFileDataAfterBadSave, originalTempFileData) {
		t.Error("Temp file was changed after a corrupted main file save")
	}
